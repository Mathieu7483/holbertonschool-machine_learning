...
class Yolo:
    ...
    def load_images(folder_path):
        ...
        if not os.path.isdir(folder_path):
            raise FileNotFoundError(
                f"The directory {folder_path} does not exist")
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        - file_name: the file name of the image (without full path)
        """
        ...
        # Display image window named after the filename only (no full path)
        cv2.imshow(file_name, image)
        key = cv2.waitKey(0)

        # 's' → save current image to detections/ then move to next image
        if key == ord('s'):
        ...
        # Close the current window and move on to the next image
    ...
    def predict(self, folder_path):
        ...
        for img, img_path, img_shape, idx in zip(images,
            ...
            # Pass only the filename (not the full path) as window title
            self.show_boxes(image=img,
                            boxes=box_preds,
