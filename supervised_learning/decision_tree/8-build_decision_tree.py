...
class Decision_Tree():
    ...
    def possible_thresholds(self, node, feature):
        """Returns the possible thresholds for
        splitting the node based on the feature"""
        values = np.unique((self.explanatory[:, feature])[node.sub_population])
        return (values[1:] + values[:-1]) / 2

    def Gini_split_criterion_one_feature(self, node, feature):
        """Returns the best threshold and the
        corresponding Gini impurity"""
        ...
        for threshold in thresholds:
            ...
            num = (np.sum(left_population) * gini_left +
                   np.sum(right_population) * gini_right)
            gini_split = num / np.sum(node.sub_population)
    ...
    def Gini_split_criterion(self, node):
        """Returns the best feature and threshold for splitting the node
